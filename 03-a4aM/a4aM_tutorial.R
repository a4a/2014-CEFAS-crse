...
# STOP!
#-------------------------------------------------------
# Including multivariate uncertainty with copulas
#-------------------------------------------------------

# Here we use a triangular copula to model the uncertainty in the parameters
# In the following example we'll use Gislason's second estimator for the shape model
# (Kenchington, 2013), M=K(\frac{L_inf}{l})^1.5$. K (Linf / l)^1.5
...
# We now have a new model that can be used for the shape model
# Use the constructor or the set method to add the new model.
# Note that we have a quite complex method now. A length based shape model from Gislason's work, Jensen's third based temperature level and a time trend depending on NAO. 
